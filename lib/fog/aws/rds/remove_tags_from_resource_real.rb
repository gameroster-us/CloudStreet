module Fog
  module AWS
    class Rds
      class RemoveTagsFromResourceReal
        ...
        def remove_tags_from_resource(rds_id, keys)
          @rds_owner_id ||= self.describe_db_instances.body["DescribeDBInstancesResult"]["DBInstances"].first["KmsKeyId"].split(':')[4]
          resource_name = "arn:aws:rds:#{@region}:#{@rds_owner_id}:db:#{rds_id}"
