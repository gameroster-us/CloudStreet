class Services::Network::NetworkLoadBalancer::AWS < Service
  include Services::ServiceHelpers::AWS
  include Behaviors::Costable::Amazon::NetworkLoadBalancer
  store_accessor :data, :region, :dns_name, :scheme, :category, :arn, :canonical_hosted_zone_id, :availability_zones, :ip_address_type, :security_groups, :subnet_ids, :deletion_protection_enabled, :access_logs_s3_enabled, :access_logs_s3_bucket, :access_logs_s3_prefix, :load_balancing_cross_zone_enabled, :listeners, :target_groups, :health_check_protocol, :health_check_port, :health_check_enabled, :hcheck_interval, :response_timeout, :healthy_threshold, :unhealthy_threshold, :health_check_path, :matcher, :tags, :service_tags
  ...
  def provides
    [
      { name: "NetworkLoadBalancer", protocol: Protocols::NetworkLoadBalancer, limit: 1, max_connections: 1024 }
    ]
  end
  ...
  def validate_for_termination
    CloudStreet.log "--------------------validating before terminating network lb-------------------------"
