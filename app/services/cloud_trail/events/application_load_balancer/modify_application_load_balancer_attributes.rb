module CloudTrail::Events::ApplicationLoadBalancer::ModifyApplicationLoadBalancerAttributes
  def process
    CTLog.info "****** Inside ModifyApplicationLoadBalancerAttributes ******"
  ...
  def get_event_attributes(event)
    ...
    lb_attributes.each do |attrs|
      ...
      elsif attrs["key"].eql?("idle_timeout.timeout_seconds")
        attributes.merge!({"idle_time_out_seconds" => attrs["value"]})
      elsif attrs["key"].eql?("deletion_protection.enabled")
        attributes.merge!({"deletion_protection_enabled" => attrs["value"]})
      elsif attrs["key"].eql?("routing.http2.enabled")
        attributes.merge!({"routing_http2_enabled" => attrs["value"]})
      elsif attrs["key"].eql?("routing.http.drop_invalid_header_fields.enabled") 
         attributes.merge!({"drop_invalid_header_fields_enabled" => attrs["value"]})
      end
    end
    return attributes
  end
